...
struct ReducerTests {
    ...
    // MARK: - Slug Propagation Through updatedAt Skip

    @Test("reconciled propagates slug even when existing link is newer (updatedAt skip)")
    func reconciledPropagatesSlugThroughUpdatedAtSkip() {
        // Simulate: activityChanged bumped updatedAt to .now, but link has no slug
        let existing = makeLink(
            id: "card_slug1",
            column: .waiting,
            sessionLink: SessionLink(sessionId: "sess_slug1"),
            updatedAt: .now
        )
        var state = stateWith([existing])

        // Reconciler discovered slug but its snapshot is 10s stale
        let reconciledLink = makeLink(
            id: "card_slug1",
            column: .waiting,
            sessionLink: SessionLink(sessionId: "sess_slug1", sessionPath: "/path/to/session", slug: "immutable-finding-mist"),
        ...
        // Slug should be propagated despite the reconciled link being "stale"
        #expect(state.links["card_slug1"]?.sessionLink?.slug == "immutable-finding-mist")
        #expect(state.links["card_slug1"]?.sessionLink?.sessionPath == "/path/to/session")
    }

    @Test("reconciled propagates slug when existing link has no sessionLink")
    func reconciledPropagatesSlugNoExistingSession() {
        // Link exists with no sessionLink at all (e.g., manual card)
        let existing = makeLink(
            id: "card_slug2",
            column: .waiting,
            updatedAt: .now
        ...
        let reconciledLink = makeLink(
            id: "card_slug2",
            column: .waiting,
            sessionLink: SessionLink(sessionId: "sess_slug2", slug: "test-slug-two"),
        ...
        // SessionLink should be propagated from reconciled version
        #expect(state.links["card_slug2"]?.sessionLink?.slug == "test-slug-two")
        #expect(state.links["card_slug2"]?.sessionLink?.sessionId == "sess_slug2")
    }

    @Test("reconciled propagates slug during isLaunching preserve path")
    func reconciledPropagatesSlugDuringLaunch() {
        // Card is mid-launch, no activity yet — will be preserved
        let existing = makeLink(
            id: "card_slug3",
            column: .inProgress,
            sessionLink: SessionLink(sessionId: "sess_slug3"),
            isLaunching: true,
            updatedAt: .now
        )
        var state = stateWith([existing])
        ...
        let reconciledLink = makeLink(
            id: "card_slug3",
            column: .waiting,
            sessionLink: SessionLink(sessionId: "sess_slug3", sessionPath: "/discovered/path", slug: "launching-slug"),
            updatedAt: .now.addingTimeInterval(-3)
        )
        let result = ReconciliationResult(
            links: [reconciledLink],
            sessions: [],
            activityMap: [:], // no activity — triggers "still launching" preserve
            tmuxSessions: []
        )
        let _ = Reducer.reduce(state: &state, action: .reconciled(result))

        // Card preserved (still launching) but slug should propagate
        #expect(state.links["card_slug3"]?.isLaunching == true)
        #expect(state.links["card_slug3"]?.sessionLink?.slug == "launching-slug")
        #expect(state.links["card_slug3"]?.sessionLink?.sessionPath == "/discovered/path")
    }

    @Test("reconciled does not overwrite existing slug with nil")
    func reconciledDoesNotClobberExistingSlug() {
        // Link already has a slug from a previous reconciliation
        let existing = makeLink(
            id: "card_slug4",
            column: .waiting,
            sessionLink: SessionLink(sessionId: "sess_slug4", slug: "existing-slug"),
            updatedAt: .now
        )
        var state = stateWith([existing])

        // New reconciliation has no slug (e.g., session file not yet parsed)
        let reconciledLink = makeLink(
            id: "card_slug4",
            column: .waiting,
            sessionLink: SessionLink(sessionId: "sess_slug4"),
            updatedAt: .now.addingTimeInterval(-5)
        )
        let result = ReconciliationResult(
            links: [reconciledLink],
        ...
        // Existing slug must survive — never overwrite with nil
        #expect(state.links["card_slug4"]?.sessionLink?.slug == "existing-slug")
    }
}
