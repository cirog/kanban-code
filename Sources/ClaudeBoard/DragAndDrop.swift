...
@Observable
class DragState {
    var draggingCard: ClaudeBoardCard?
    var sourceColumn: ClaudeBoardColumn?
            ...
            LazyVStack(spacing: 8) {
                ForEach(cards) { card in
                    ...
                    CardView(
                        ...
                        onSetProject: { projectId in onSetProject(card.id, projectId) }
                    )
                    // Merge highlight
                    .overlay(
...
struct ColumnDropDelegate: DropDelegate {
    ...
    let canDropCard: (ClaudeBoardCard, ClaudeBoardColumn) -> Bool
    let onMoveCard: (String, ClaudeBoardColumn) -> Void
    ...
    private func updateReorderTarget(at location: CGPoint, source: ClaudeBoardCard) {
        dragState.mergeTargetId = nil
    ...
    private func updateMergeTarget(at location: CGPoint, source: ClaudeBoardCard) {
        dragState.reorderTargetId = nil
