...
@main
struct ClaudeBoardApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) var appDelegate

    var body: some Scene {
        Window("Kanban Code", id: "main") {
            ContentView()
                .frame(minWidth: 920, minHeight: 500) // 4×180 cols + 180 terminal min + padding
        }
        .defaultSize(width: 1200, height: 700)
        .commands {
            CommandGroup(replacing: .newItem) {
                Button("New Task") {
                    NotificationCenter.default.post(name: .claudeBoardNewTask, object: nil)
                }
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate, @unchecked Sendable {
    ...
    func windowShouldClose(_ sender: NSWindow) -> Bool {
        NotificationCenter.default.post(name: .claudeBoardCloseTerminalTab, object: nil)
        return false
    }

    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        // Post notification — ContentView checks store state for managed sessions
        NotificationCenter.default.post(name: .claudeBoardQuitRequested, object: nil)
            ...
            if url.host == "card",
               let cardId = url.pathComponents.dropFirst().first, !cardId.isEmpty {
                NotificationCenter.default.post(
                    name: .claudeBoardSelectCard, object: nil,
    ...
    func userNotificationCenter(
        ...
        if let cardId = response.notification.request.content.userInfo["cardId"] as? String {
            NotificationCenter.default.post(name: .claudeBoardSelectCard, object: nil, userInfo: ["cardId": cardId])
        }
...
extension Notification.Name {
    static let claudeBoardNewTask = Notification.Name("claudeBoardNewTask")
static let claudeBoardHookEvent = Notification.Name("claudeBoardHookEvent")
    static let claudeBoardHistoryChanged = Notification.Name("claudeBoardHistoryChanged")
    static let claudeBoardSettingsChanged = Notification.Name("claudeBoardSettingsChanged")
    static let claudeBoardSelectCard = Notification.Name("claudeBoardSelectCard")
    static let claudeBoardQuitRequested = Notification.Name("claudeBoardQuitRequested")
    static let claudeBoardSelectTerminalTab = Notification.Name("claudeBoardSelectTerminalTab")
    static let claudeBoardCloseTerminalTab = Notification.Name("claudeBoardCloseTerminalTab")
}
