...
struct ContentView: View {
    ...
    private var boardWithHandlers: some View {
        boardWithAlerts
            ...
            .onReceive(NotificationCenter.default.publisher(for: .claudeBoardNewTask)) { _ in
                presentNewTask()
            }
            .onReceive(NotificationCenter.default.publisher(for: .claudeBoardHookEvent)) { _ in
                Task {
            ...
            .onReceive(NotificationCenter.default.publisher(for: .claudeBoardSelectCard)) { notification in
                if let cardId = notification.userInfo?["cardId"] as? String {
                    store.dispatch(.selectCard(cardId: cardId))
                }
            }
            .onReceive(NotificationCenter.default.publisher(for: .claudeBoardSettingsChanged)) { _ in
                Task {
                    await store.loadSettingsAndCache()
                    await store.reconcile()
                    applyAppearance()
            ...
            .onReceive(NotificationCenter.default.publisher(for: .claudeBoardQuitRequested)) { _ in
        ...
        boardWithHandlers
            ...
            .toolbar {
                ...
                ToolbarItemGroup(placement: .principal) {
                    UsageBarView(label: "5h", utilization: usageData.fiveHourUtilization, resetsAt: usageData.fiveHourResetsAt, resetDisplay: .countdown, windowDuration: 5 * 3600)
                    UsageBarView(label: "7d", utilization: usageData.sevenDayUtilization, resetsAt: usageData.sevenDayResetsAt, resetDisplay: .weekday, windowDuration: 7 * 24 * 3600)
                }
    ...
    private nonisolated func watchHookEvents(path: String) async {
        ...
        ClaudeBoardLog.info("watcher", "File watcher started for hook-events.jsonl")
        for await _ in events {
            ClaudeBoardLog.info("watcher", "hook-events.jsonl changed")
            NotificationCenter.default.post(name: .claudeBoardHookEvent, object: nil)
        }
        ClaudeBoardLog.info("watcher", "File watcher loop exited (cancelled?)")
    ...
    /// In-app settings changes post `.claudeBoardSettingsChanged` directly.
    ...
    private func navigateCard(_ direction: NavDirection) {
        ...
        var currentCol: ClaudeBoardColumn?
        var currentIndex = 0
        for col in columns {
            let colCards = store.state.cards(in: col)
    ...
    private func selectTerminalTab(at index: Int) {
        NotificationCenter.default.post(
            name: .claudeBoardSelectTerminalTab,
            object: nil,
    ...
    private func handleColumnBackgroundClick(_ column: ClaudeBoardColumn) {
    ...
    private func createManualTask(prompt: String, projectPath: String?, title: String? = nil, startImmediately: Bool = false, images: [ImageAttachment] = []) {
        ...
        store.dispatch(.createManualTask(link))
        ClaudeBoardLog.info("manual-task", "Created manual task card=\(link.id.prefix(12)) name='\(name)' project=\(projectPath ?? "nil") startImmediately=\(startImmediately)")
    ...
    private func createManualTaskAndLaunch(prompt: String, projectPath: String?, title: String? = nil, createWorktree: Bool, runRemotely: Bool, skipPermissions: Bool = true, commandOverride: String? = nil, images: [ImageAttachment] = [], assistant: CodingAssistant = .claude) {
        ...
        store.dispatch(.createManualTask(link))
        ClaudeBoardLog.info("manual-task", "Created & launching task card=\(link.id.prefix(12)) name='\(name)' project=\(effectivePath)")
    ...
    private func handleDrop(cardId: String, to column: ClaudeBoardColumn) {
        guard let card = store.state.cards.first(where: { $0.id == cardId }) else { return }
    ...
    private func executeLaunch(cardId: String, prompt: String, projectPath: String, worktreeName: String? = nil, runRemotely: Bool = true, skipPermissions: Bool = true, commandOverride: String? = nil, images: [Any] = [], assistant: CodingAssistant = .claude) {
        ...
        Task {
            do {
                ...
                ClaudeBoardLog.info("launch", "Tmux session created: \(tmuxName)")
    ...
    private func executeMigration(cardId: String, targetAssistant: CodingAssistant) async {
        ...
        do {
            ...
            ClaudeBoardLog.info("migrate", "Migration failed for card=\(cardId.prefix(12)): \(error.localizedDescription)")
        }
    }
    ...
    private func forkCard(cardId: String) {
        ...
        Task {
            do {
                ...
                ClaudeBoardLog.error("fork", "Fork failed: \(error)")
            }
        }
    }
    ...
    private func executeResume(cardId: String, runRemotely: Bool = false, skipPermissions: Bool = true, commandOverride: String?, assistant: CodingAssistant = .claude) {
        ...
        Task {
            do {
                ...
                ClaudeBoardLog.info("resume", "Resume launched for card=\(cardId.prefix(12)) actualTmux=\(actualTmuxName)")

                store.dispatch(.resumeCompleted(cardId: cardId, tmuxName: actualTmuxName))
            } catch {
                ClaudeBoardLog.info("resume", "Resume failed for card=\(cardId.prefix(12)): \(error.localizedDescription)")
