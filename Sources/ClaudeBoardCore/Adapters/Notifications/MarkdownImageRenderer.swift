    ...
    public static func renderToImage(markdown: String) async -> Data? {
        ...
        do {
            ...
            guard pandocResult.succeeded else {
                ClaudeBoardLog.error("markdown-render", "pandoc failed: \(pandocResult.stderr)")
                return nil
            }
            ...
            guard imgResult.succeeded else {
                ClaudeBoardLog.error("markdown-render", "wkhtmltoimage failed: \(imgResult.stderr)")
                return nil
            }

            return try Data(contentsOf: URL(fileURLWithPath: imgPath))
        } catch {
            ClaudeBoardLog.error("markdown-render", "render failed: \(error)")
            return nil
        }
    }
