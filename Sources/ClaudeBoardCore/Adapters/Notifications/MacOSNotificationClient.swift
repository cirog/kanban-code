...
public final class MacOSNotificationClient: NotifierPort, @unchecked Sendable {
    ...
    public func sendNotification(title: String, message: String, imageData: Data?, cardId: String?) async throws {
        ...
        guard settings.authorizationStatus == .authorized || settings.authorizationStatus == .provisional else {
            ClaudeBoardLog.info("notify", "Notification skipped: authorization=\(settings.authorizationStatus.rawValue)")
            return
        }
        ...
        if let imageData {
            ...
            do {
                ...
                ClaudeBoardLog.info("notify", "Image attachment failed: \(error)")
            }
        }
        ...
        do {
            ...
            ClaudeBoardLog.info("notify", "macOS notification delivered: \(title)")
        } catch {
            ClaudeBoardLog.info("notify", "Notification failed: \(error)")
            throw error
        }
    }
