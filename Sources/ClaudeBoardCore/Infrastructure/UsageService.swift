...
public actor UsageService {
    ...
    public func start() {
        timer = Task {
            ...
            if cachedData.lastFetchError != nil {
                ClaudeBoardLog.info("usage", "First fetch failed, retrying in 10s...")
    ...
    private func fetchUsage() async {
        guard let token = getOAuthToken() else {
            ClaudeBoardLog.info("usage", "Failed to get OAuth token from keychain")
        ...
        do {
            ...
            ClaudeBoardLog.info("usage", "Network error: \(error.localizedDescription)")
            cachedData = UsageData(
                fiveHourUtilization: cachedData.fiveHourUtilization,
                fiveHourResetsAt: cachedData.fiveHourResetsAt,
                sevenDayUtilization: cachedData.sevenDayUtilization,
        ...
        if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode != 200 {
            ...
            ClaudeBoardLog.info("usage", "HTTP \(httpResponse.statusCode): \(body.prefix(200))")
            cachedData = UsageData(
                fiveHourUtilization: cachedData.fiveHourUtilization,
                fiveHourResetsAt: cachedData.fiveHourResetsAt,
                sevenDayUtilization: cachedData.sevenDayUtilization,
        ...
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            ClaudeBoardLog.info("usage", "Failed to parse JSON response")
            return
        }
        ...
        ClaudeBoardLog.info("usage", "Fetched: 5h=\(newData.fiveHourUtilization)% 7d=\(newData.sevenDayUtilization)%")
        cachedData = newData
    }
